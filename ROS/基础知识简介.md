...
### 1.5 小结与点评
对ROS2有了简单地认识，并成功地搭建了虚拟机和ROS2开发环境。接着通过海龟模拟器对ROS2的通信机制和调试工具有了一定的了解。<br/>


## 2、ROS2基础入门---从第一个节点开始
学习在哪编写ROS2程序，以及如何编译代码生成可执行文件。<br/>

### 2.1 编写你的第一个节点
节点除了可以订阅和发布话题外，还可以使用服务、配置参数和执行动作等。
#### 2.1.1 python示例
```python
import rclpy    #导入python版本客户端库rclpy
from rclpy.node import Node     #从rclpy库的node模块中导入Node类

def main():
    rclpy.init()   #为通信分配资源
    node = Node("python_node")  #创建一个名为python_node的Node类实例，然后通过实例进行订阅和发布话题
    node.get_logger().info('你好python节点！')  #通过node实例调用get_logger()获取日志记录器，接着调用日志记录器的info方法输出一句话。
    rclpy.spin(node)     #启动节点，不断循环检查被其运行的节点是否收到新的话题数据等事件，直到该节点被关闭为止。
    rclpy.shutdown()   #用于清理分配的资源并确认节点是否被关闭。

if __name__=="__main__":
    main()

```
终端命令：ros2 node list     ===>  /python_node     查看当前节点
